...
export const PageSelector = <K extends { label: string, url: string }>({ options, selected, onSelected }: PageSelectorProps<K>) =>
    <div style={{ margin: "16px 0", justifyContent: 'center', display: 'flex', flexWrap: 'wrap' }}>
        {options.map((el, i) => {
            const isSelected = selected == el

            return <a key={i} onClick={() => onSelected(el)} style={{ marginBottom: 32 }} className={isSelected ? styles.selected : styles.selector}>
                <h2 style={{
                    color: 'white',

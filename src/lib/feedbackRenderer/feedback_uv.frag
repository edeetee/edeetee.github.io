...
void main () {
    ...
    float mouseStrength = pow(max(1.0-mouseLength*2.0, 0.0), 3.0);
    ...
    textureUV += snoise32(vec3(aspectUv*1.0, t*0.05))*0.01*speed;
    ...
    outUv += pow(snoise32(vec3(aspectUv*100.0+vec2(1234.1232), t*100.323)), vec2(10.0))*0.1*speed;

    //alpha starts at 0
    outUv = mix(uv, outUv, (1.0-0.1*speed)*textureColor.a);
